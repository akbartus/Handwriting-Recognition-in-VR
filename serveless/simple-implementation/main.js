...
async function getSampleText() {
  await fetch("word-list.txt")
    ...
    .then((text) => {
      ...
      document.getElementById("outputText").innerHTML = resultingText.toString().replace(/,/g, ' ')
...
class Model {
  ...
  loadModel() {
    ...
    return tf.loadLayersModel("models/model/model.json").then((model) => {
      this._model = model;
  ...
  // Do Batch recognition
    ...
    function doBatchRecognition() {
      ...
      setTimeout(function () {
        ...
        document.querySelector("#outputText").innerHTML = recognizedTxt.replace(/0/g, "O");
...
function onFileSelected(event) {
  ...
  while (parentElement.firstChild) {
    ...
    document.getElementById("displayedImage").setAttribute("style", "height: 160px; border: 2px solid #000000;");
